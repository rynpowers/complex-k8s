...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=rynpowers/complex-client:$SHA
kubectl set image deployments/server-deployment server=rynpowers/complex-server:$SHA
kubectl set image deployments/worker-deployment worker=rynpowers/complex-worker:$SHA
